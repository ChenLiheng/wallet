...
import game.fair.ethwallet.crypto.SCrypt;

import static game.fair.ethwallet.wallet.SecureRandomUtils.secureRandom;
...
public class MyWallet {
    ...
    public static MyWalletFile create(String password, ECKeyPair ecKeyPair, int n, int p)
            throws CipherException {

        byte[] salt = generateRandomBytes(32);
        Log.i("MyWallet","salt-----"+salt.toString());

        byte[] derivedKey = generateDerivedScryptKey(
                password.getBytes(Charset.forName("UTF-8")), salt, n, R, p, DKLEN);
        Log.i("MyWallet","derivedKey-----"+derivedKey.toString());

        byte[] encryptKey = Arrays.copyOfRange(derivedKey, 0, 16);
        Log.i("MyWallet","encryptKey-----"+encryptKey.toString());
        byte[] iv = generateRandomBytes(16);

        byte[] privateKeyBytes =
                Numeric.toBytesPadded(ecKeyPair.getPrivateKey(), 32);
        Log.i("MyWallet","privateKeyBytes-----"+privateKeyBytes.toString());

        byte[] cipherText = performCipherOperation(
                Cipher.ENCRYPT_MODE, iv, encryptKey, privateKeyBytes);
        Log.i("MyWallet","cipherText-----"+cipherText.toString());

        byte[] mac = generateMac(derivedKey, cipherText);
        Log.i("MyWallet","mac-----"+mac.toString());
        return createWalletFile(ecKeyPair, cipherText, iv, salt, mac, n, p);
    }

    public static MyWalletFile createStandard(String password, ECKeyPair ecKeyPair)
            throws CipherException {
        return create(password, ecKeyPair, N_STANDARD, P_STANDARD);
    }

    public static MyWalletFile createLight(String password, ECKeyPair ecKeyPair)
            throws CipherException {
        return create(password, ecKeyPair, N_LIGHT, P_LIGHT);
    }

    private static MyWalletFile createWalletFile(
            ECKeyPair ecKeyPair, byte[] cipherText, byte[] iv, byte[] salt, byte[] mac,
            int n, int p) {

        MyWalletFile walletFile = new MyWalletFile();
        walletFile.setAddress(Keys.getAddress(ecKeyPair));

        MyWalletFile.Crypto crypto = new MyWalletFile.Crypto();
        crypto.setCipher(CIPHER);
        crypto.setCiphertext(Numeric.toHexStringNoPrefix(cipherText));
        walletFile.setCrypto(crypto);

        MyWalletFile.CipherParams cipherParams = new MyWalletFile.CipherParams();
        cipherParams.setIv(Numeric.toHexStringNoPrefix(iv));
        crypto.setCipherparams(cipherParams);

        crypto.setKdf(SCRYPT);
        MyWalletFile.ScryptKdfParams kdfParams = new MyWalletFile.ScryptKdfParams();
    ...
    public static ECKeyPair decrypt(String password, MyWalletFile walletFile)
            throws CipherException {

        validate(walletFile);

        MyWalletFile.Crypto crypto = walletFile.getCrypto();
        ...
        MyWalletFile.KdfParams kdfParams = crypto.getKdfparams();
        if (kdfParams instanceof MyWalletFile.ScryptKdfParams) {
            MyWalletFile.ScryptKdfParams scryptKdfParams =
                    (MyWalletFile.ScryptKdfParams) crypto.getKdfparams();
            ...
            derivedKey = generateDerivedScryptKey(
                    password.getBytes(Charset.forName("UTF-8")), salt, n, r, p, dklen);
        } else if (kdfParams instanceof MyWalletFile.Aes128CtrKdfParams) {
            MyWalletFile.Aes128CtrKdfParams aes128CtrKdfParams =
                    (MyWalletFile.Aes128CtrKdfParams) crypto.getKdfparams();
    ...
    static void validate(MyWalletFile walletFile) throws CipherException {
        MyWalletFile.Crypto crypto = walletFile.getCrypto();
